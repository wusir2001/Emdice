...
bool CQTool::GetStrangerInfo(int auth, int64_t qqid, CQ_TYPE_QQ& Stranger)
{
	std::string encodeStr = CQ_getStrangerInfo(auth, qqid, false);
	if (0 == encodeStr.size())
	{
		return false;
	}
	std::string decodeStr = base64_decode(encodeStr);
	if (decodeStr.size() < 18)
	{
		CQ_addLog(auth, CQLOG_ERROR, "CQTool::GetStrangerMemberInfo", "decodeStr.size() < 18");
		return false;
	}
	return CQ_Tools_TextToStrangerMember(decodeStr, Stranger);
}

bool CQTool::CQ_Tools_TextToStrangerMember(std::string decodeStr, CQ_TYPE_QQ& Stranger)
{
	int i = 0;
	int count = 8;
	Stranger.QQID = CoolQ_Tools_GetNum(i, count, decodeStr);
	i += count;

	Stranger.nick = CoolQ_Tools_GetStr(i, decodeStr);
	i += 2 + Stranger.nick.size();

	count = 4;
	Stranger.sex = (int)CoolQ_Tools_GetNum(i, 4, decodeStr);
	i += count;

	count = 4;
	Stranger.age = (int)CoolQ_Tools_GetNum(i, 4, decodeStr);
	i += count;
